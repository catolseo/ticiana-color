...
В текущей `D:\backup\Ticiana\AdsPro\book\` файлы зашифрованы XOR со
**сложным позиционным ключом** (зависит от префикса имени файла, см. ниже).
Расшифровка не реализована — мы используем январский бэкап.

### Заметки про шифрование (для будущих попыток reverse engineering)

- Все 207 файлов начинаются с одинаковых 11 байт: `c0 c4 bf 3a 35 3b 25 2b 13 19 00`
- Файлы с одинаковым префиксом продукта (`b11_*`, `c11_*`) совпадают первыми 32 байтами,
  включая позицию, где обычно стоит код продукта в незашифрованном файле.
  Это значит, что ключ зависит от имени файла — но не сам по себе, а через
  плейн-текст (заголовок CSV содержит код).
- Простой constant-XOR не подходит (проверено).
- ADSPro_en.pdf только упоминает «возможность защитить/зашифровать данные книги формул»
  без описания алгоритма.
- На бэкапе Jan-2024 шифрование ещё не было включено — поэтому он открыт.
- В `D:\backup\Ticiana\Резервные копии и обновления\book_обновление.rar` (Jan 2024)
  возможно лежит более свежий открытый дамп — нужен `unrar` (rarfile pip-пакет требует
  внешний tool). Не проверял.
...
- **Расшифровать боевые `D:\backup\Ticiana\AdsPro\book\*.csv`** — это даст более свежие
  формулы (бэкап Jan-2024 устарел, в нём нет, например, `Ticiana Deluxe BELUCCI`).
  Подходы: дизассемблер `ADSPro.exe`, динамический трейс при запуске под отладчиком,
  или покопаться в `book_обновление.rar` если там открытый дамп.
...
- **GitHub Pages деплой** — сейчас репозитория нет. Когда будет — `gh-pages` ветка
  или Actions; при деплое не забыть про `.gitignore` (исключает `data/book/` из коммита,
  если решите хранить только сгенерированные `formulas/*.js`).
